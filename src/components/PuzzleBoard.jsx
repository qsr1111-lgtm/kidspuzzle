...
function getTrayColumns(grid) {
  if (grid === 2) return "repeat(2, minmax(0, 1fr))";
  if (grid === 3) return "repeat(3, minmax(0, 1fr))";
...
export default function PuzzleBoard({ puzzle, level, progress, onBack, onComplete }) {
  ...
  return (
    <div className="h-[100dvh] max-h-[100dvh] overflow-hidden px-3 py-3 sm:px-5">
      <DndContext
        sensors={sensors}
      ...
      >
        <div className="flex h-full flex-col gap-3">
          <header className="flex shrink-0 items-center justify-between gap-3">
            <button
              onClick={onBack}
              className="flex h-[54px] w-[54px] items-center justify-center rounded-[18px] bg-white text-2xl font-black shadow-lg active:scale-95 md:h-[60px] md:w-[60px]"
            >
              ←
            </button>

            <div className="text-center">
              <h2 className="text-2xl font-black leading-none md:text-4xl">
                {puzzle.title}
              </h2>
            ...
            <button
              ...
              className="flex h-[54px] w-[54px] items-center justify-center rounded-[18px] bg-white text-xl font-black shadow-lg active:scale-95 md:h-[60px] md:w-[60px]"
            >
              👀
            </button>
          </header>

          <div className="shrink-0 text-center text-base font-black text-[#7b4a24] md:text-xl">
            Перетащи кусочки в пустые места
          </div>

          <section className="grid min-h-0 flex-1 grid-cols-1 gap-3 md:grid-cols-[minmax(360px,1fr)_minmax(260px,430px)] md:items-stretch">
            <div className="flex min-h-0 items-center justify-center">
              <div className="aspect-square h-full max-h-full max-w-full rounded-[28px] bg-white/80 p-2 shadow-2xl md:p-3">
                <div
            ...
            <aside className="flex min-h-0 flex-col rounded-[28px] bg-white/70 p-3 shadow-xl md:p-4">
              <div className="mb-3 grid grid-cols-[1fr_auto_1fr] items-center gap-2">
                <button
                  onClick={restartGame}
                  className="min-h-[50px] rounded-[18px] bg-[#ffcf5a] px-3 text-base font-black shadow-md active:scale-95 md:text-lg"
                >
                  🔄 Сначала
                </button>
                ...
                <button
                  ...
                  className="min-h-[50px] rounded-[18px] bg-[#8ee06e] px-3 text-base font-black shadow-md active:scale-95 md:text-lg"
                >
                  👀 Помощь
                </button>
              </div>
              ...
              <div className="min-h-0 flex-1">
                <div
                  className="grid h-full content-center gap-3 md:gap-4"
                  style={{
